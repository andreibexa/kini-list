...
const getDiscoverMovie = async <T>(countryIso: string | undefined, providers: Provider[], genreId?: number) => {
    try {
        ...
        const {data} = await apiClientV3
            .get<T>(`/discover/movie?watch_region=${countryIso || 'us'}&with_watch_providers=${pipeProviderIds}&with_genres=${genreId || ''}`);
